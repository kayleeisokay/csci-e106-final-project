...
abstract: 
  We aimed to develop models for predicting house prices in Kings County, USA using statistical modeling and machine learning approaches. Our dataset contained historical home sales prices of 21,613 houses in Kings County, USA (May 2014-May 2015), out of which we randomly sampled 70% for training and 30% for testing. We selected several significant features using feature selection methods to build the models. Seven different linear regression models were developed using R and compared against each other, with lasso and elastic net regression achieving the highest adjusted R-squared (0.7745). In addition, we developed alternative models using random forest, which achieved an adjusted R-squared value of 0.65. 
...
Using this framework, we built several different models using linear regression and random forest methods. First, we built a baseline OLS linear regression model using all available variables. Then we performed stepwise variable selection to ensure the inclusion of only significant variables. Diagnostic tests at this point indicated issues with normality assumption, multicollinearity, and constant variance assumption. These issues were largely resolved by a log transformation of the dependent variable as suggested by box-cox analysis. We further built models using ridge, lasso, and elastic net regression, as well as a robust regression to account for any effects of outliers. As an additional comparison, we also built a random forest model and employed hyperparameter tuning procedures. 

Finally, the performance of each model was evaluated by its accuracy in predicting house prices in the test set, specifically by examining the adjusted R-squared and MSE of predicted values. Based on model performance and properties, we propose that the best predictive model is our elastic net model (adjusted R-squared = 0.775). Our analysis indicated that the most important factors that influence house prices in King County, USA are bathrooms, view, grade, latitude, year built, year of purchase, and number of floors. Our top model may be useful for estimating property prices for home buyers, sellers, or property market professionals. It can also contribute to research on key factors contributing to home prices. Importantly, we expect the model to be valid only in geographical locations within King County and within a limited time frame into the future. Therefore, we have also detailed a model monitoring plan to detect substantial future deviations of our model.
...
Training and test data sets were generated in the previous section. ID was also previously removed as an irrelevant variable to price. We engineered some additional variables such as quality-adjusted features. For example, we reasoned that each additional square feet for a house located at the waterfront may predict a higher price increase compared to a house not located at the waterfront, so we created the variable "sqft_adj_waterfront". We also introduced square terms for "floor" and "sqft_living" with the idea that these may be variables with diminishing returns.
...
First, we evaluated the base model performance.
...
Next, we applied stepwise variable selection.

```{r part IV - Stepwise both ways}
library(olsrr)
stepwise_model = ols_step_both_p(baseline.model, pent=0.35, prem=0.05)
plot(stepwise_model$adjr,ylab="Adjusted R-squared",xlab="step_index",main="Adjusted R-squared in stepwise variable selection")
stepwise_final = stepwise_model$model
summary(stepwise_final)
```
Stepwise reduced the variables as intended. Following, I`ll identify which variables were selected (and create a new dataframe to which we will use in our next interactions) and identify the ones dropped.
...
After this, we checked multi-collinearity with VIF analysis.
...
We got 3 variables above our cutoff 10 Vif value: sqft_living, sqft_adj_grade and sqft_adj_condition. We have compared the model performance with or without removing these variables and found that it had minimal effect. We restored them when we used other remedial measures, like ridge regression.
...
#selected_train_df2 = selected_train_df[, !names(selected_train_df) %in% VIF_eliminated]
#selected_test_df2 = selected_test_df[, !names(selected_test_df) %in% VIF_eliminated]

#selected_linear_model2 = lm(price ~ . , data = cbind(price = kc.house.train.y, selected_train_df2))

#pred_selected <- predict(selected_linear_model2, selected_test_df2)
#act_selected <- kc.house.test.y
#n_selected <- dim(model.matrix(selected_linear_model2)[, -1])[1]
#p_selected <- dim(model.matrix(selected_linear_model2)[, -1])[2]

#performance_selected_vif = cbind(CalcTestMetrics(pred_selected, act_selected, n_selected, p_selected))
#print(performance_selected_vif)
```
...
The diagnostic plots indicated issues with normality assumption, as there were many points especially on the higher end that deviated from expected quantiles. The scale-location plot showed violation of constant variance assumption. Overall the data seems heavily right-skewed, so we next performed box-cox analysis to check for any necessary transformations.
...
Box-cox value indicates optimal lambda at a value very close to zero. Although zero falls just outside the 95% confidence interval, for practical and interpretability reasons we chose to perform log-transformation on the dependent variable. After transforming the model (log of the price), the assumptions seem to be fine. Just to be sure on normality, let's do some tests.
...
# Summarize statistics for exponetiated enet predictions
...
1) model stability (e.g. comparing prediction errors, MSE, and mean price values on a new batch of data to older batches by T test. Also includes normality testing on residuals from new batch of predictions.)

2) prediction performance over time (e.g. comparing pseudo R2 and MSE values over the period of time since model has been in use. Deviations may suggest changing market conditions affecting model performance)

3) incremental data quality (e.g. monitor if there are outliers/ influential points in incoming new data using tools like Residuals vs Leverage plot)
...
```{r echo=TRUE}
# Helper function to define appropriate window for "old data"
...
# Simulate existing model training and performance test 
# (In this example, we use data from 2014/01-2015/01 for training and 2015/01-2015/02 as testing) 
...
If any metrics of the production model is worse than the baseline production model, we will trigger a slack message.
...
This project aims to develop statistical models for predicting house prices in Kings County, USA. Using our training dataset, we built seven different linear regression models and compared them against each other, with our top models using lasso regression and elastic net regression achieving the highest adjusted R-squared (>0.7745) and lowest MSE values. We developed alternative models using random forest, which achieved an adjusted R-squared value of 0.65. 

In the table here we show the performance and coefficient comparisons between linear models again. Aside from baseline and stepwise selection models (which are not log-transformed), all other linear models show very comparable performance. Considering that elastic net uses a mix of L1 and L2 regularization and performs well with complex data, we believe that it is our champion model. Its high adjusted R-squared value means that it can explain over 77% of the variation in log-transformed house prices seen in the dataset, and its low MSE value indicates its accuracy when predicting house prices using test data. Compared to the random forest models, we believe the elastic net model would be robust over a longer period of time and offer higher interpretability, and so the elastic net is our model of choice.
