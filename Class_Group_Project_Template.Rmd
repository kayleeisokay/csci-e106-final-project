...
transform_price <- function(kc.house.df){
  ...
  kc.house.df$price <- gsub("\\$", "", kc.house.df$price)
  kc.house.df$price <- gsub("\\,", "", kc.house.df$price)
  price <- as.double(kc.house.df$price)
...
transform_date <- function(kc.house.df){
  ...
  year <- as.integer(substring(kc.house.df$date, 1, 4))
  month <- as.integer(substring(kc.house.df$date, 5, 6))
...
#Ideas
#Age of home at sale
#- if renovated then restart the clock at renovation
```
...
kc.house.df$year <- transform_date(kc.house.df)[["year"]]
kc.house.df$month <- transform_date(kc.house.df)[["month"]]
...
kc.house.train <- kc.house.df [sample, ]
kc.house.test<- kc.house.df [!sample, ]
```
...
#baseline model
baseline.model <- lm(price ~ . , data = kc.house.train)
...
pred <- predict(baseline.model, kc.house.test)
act <- kc.house.test[, response]
