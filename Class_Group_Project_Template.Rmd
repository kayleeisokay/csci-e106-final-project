...
```{r}
#baseline model
kc.house.train.y <- as.numeric(kc.house.train.y)
...
performance_baseline = cbind(CalcTestMetrics(pred, act, n, p))
...
dropped_variables
```
Seems like we dropped 12 variables from our original 33. Of those 12, 10 were dummy month variables we generated, one was the variable we used to split suburbs from the city, and the last one was sqft_above.
...
library(MASS)
boxcox(selected_linear_model,lambda=seq(-1,1,0.01))
transformed_model = lm(log(price) ~ . , data = cbind(price = kc.house.train.y, selected_train_df))
summary(transformed_model)
plot(transformed_model)
```
After transforming the model (log of the price), the assumptions seem to be fine. Just to be sure on normalitu, let's do some tests.
...
As well, P-value of test is 2.2e-16 which is below our alpha of 0.05 , confirming that we should reject $H_0$. Variances are not constant, so normality is still an issue.
...
performance_transformed = cbind(CalcTestMetrics(pred_transformed, act_transformed, n_transformed, p_transformed))
...
performance_lasso = cbind(CalcTestMetrics(pred_lasso, act_lasso, n_lasso, p_lasso))
...
performance_enet = cbind(CalcTestMetrics(pred_enet, act_enet, n_enet, p_enet))
...
performance_robust = cbind(CalcTestMetrics(pred_robust, act_robust, n_robust, p_robust))
print(performance_robust)

```
With robust performance, it does not seem that outliers matter much (after trasnformation at least)
