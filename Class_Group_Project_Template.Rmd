...
```{r section3}
set.seed(1023)
kc.house.df <- read.csv("KC_House_Sales.csv")

response <- "price"

#Dropping variables as specified
drop.vars <- c("id", "lat", "long")
kc.house.df <- dplyr::select(kc.house.df, -drop.vars)
...
kc.house.df$price <- transform_price(kc.house.df)

kc.house.df$year_sold <- transform_date(kc.house.df)[["year_sold"]]
kc.house.df$month_sold <- transform_date(kc.house.df)[["month_sold"]]
kc.house.df$date <- NULL
...
kc.house.train.X <- kc.house.df[sample, -which(names(kc.house.df) %in% c(response))]
kc.house.train.y <- kc.house.df [sample, response]

kc.house.test.X <- kc.house.df [!sample, -which(names(kc.house.df) %in% c(response))]
kc.house.test.y <- kc.house.df [!sample, response]
```
...
```
#TODO
1. Add plots (e.g. scale location, residual vs fitted)
2. Normality plots
