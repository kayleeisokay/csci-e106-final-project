...
drop.vars <- c("lat", "long")
kc.house.df <- dplyr::select(df, -drop.vars) 
...
```{r echo = FALSE, section7helperfunction}
...
prev_train_data <- prev_train_data %>% select(-contains("Month"))
prev_test_data <- prev_test_data %>% select(-contains("Month"))
...
curr_train_data <- curr_train_data %>% select(-contains("Month"))
curr_test_data <- curr_test_data %>% select(-contains("Month"))
...
roll_model <- function(cutoff_date, form){
  ...
  train_data <- train_data %>% select(-contains("Month"))
  ...
  test_data <- test_data %>% select(-contains("Month"))
