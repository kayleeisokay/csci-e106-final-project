...
# Flora: Commenting this chunk out for deletion as I have copied over code from Part IV
# Data cleaning (date, price)
#HouseSales$date = gsub("T000000", "", HouseSales$date)
#HouseSales$date = as.numeric(HouseSales$date)
# HouseSales$date = ymd(HouseSales$date)
#HouseSales$price = gsub("[$,]", "", HouseSales$price)
...
```{r section3a}
set.seed(1023)
# kc.house.df <- read.csv("KC_House_Sales.csv") # Flora: commenting this out as dataset has been read in as HouseSales (line 32)

response <- "price"

#Dropping variables as specified
drop.vars <- c("id", "lat", "long")
kc.house.df <- dplyr::select(HouseSales, -drop.vars) # Flora: Calling HouseSales in select() here (to align with line 32)

#Renaming to conform to unified convention
kc.house.df <- kc.house.df %>% rename("year_built" = "yr_built")
```

```{r section3b, eval=FALSE, include=FALSE}
# Flora: suggest removing this chunk since it is now duplicated from Part II
# Feature Transformations
#transform_price <- function(kc.house.df){
  #Converts price from string to double
#  price <- parse_number(kc.house.df$price)
  
#  return (price)
#}

#transform_date <- function(kc.house.df){
  #Converts date to month and year
#  year_sold <- as.integer(substring(kc.house.df$date, 1, 4))
#  month_sold <- as.integer(substring(kc.house.df$date, 5, 6))
  
#  return (list(year_sold = year_sold, month_sold = month_sold))
#}
```
...
# Flora: I modified and moved price/year/month transformation code to Part II
...
#  nrow(kc.house.df), 
#  replace=TRUE, 
#  prob=c(train_prop, test_prop)
#)

# Flora: I have commented out the train/test splitting code here and replaced with similar code from Part V instead. That code is more accurate in selecting 70% of samples, which should be 15129 samples.
