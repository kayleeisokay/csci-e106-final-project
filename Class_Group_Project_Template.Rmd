...
- compare current model predictions on previous week's model, if ks test is significant alert slack
- checks distribution of predictions
2. Adjusted R2 over time
- keep a running record of adjusted r squared, make sure it doesn't drop below baseline model, if so send a slack alert
- Assume daily runs
3. Quality checks on incoming data
- t-test on means
- chi square test on variances
- diagnostics on influential points (no action needed just dashboard)
- distribution of the features have to be normal (shapiro-wilk test)
3. Model Checks
- hetereoskedasticity test - less important because doesn't affect predictions
- MC - less important because doesn't affect predictions
4. Pipeline faults
- default to old data (only need to write this out)
5. Training data sampling
- use multiple folds

Technical points
- Store models in rds files

Scoping
- Assume curr model is model trained on all data
- previous model is model trained on all data minus the most recent month
   - because of data leakage 70% of data will be 2014 and rest will be 2015
- KS test plot
- Create 7 different training sets, plot adjusted squared over month 1, 2, ..
- Function to check data quality (pick the previous day's training data)
  - t-test
  - chisquare
  - can tune to see which lookback window is optimal because seasonal data naturally drifts, as long as model accounts we are ok
  
```{r section7}
previous.model <- NULL
current.model <- NULL

previous.pred <- predict(previous.model)
curr.pred <- predict(current.model)

# H0: same distribution
# H1: not the same distribution
ks.test(previous.pred, curr.pred)
```
